...
// Handles all the calls to the backend
...
export interface Exchange {
  id: number;
  rate: number;
  marketRate?: number;     // user-provided rate
  spread?: number;        // (marketRate - rate) / marketRate * 100
...
export async function getCategories(category_type?: 'expense' | 'income'): Promise<Category[]> {
  ...
  if (!response.ok) throw new Error('Failed to get the list');
  return response.json();
}
